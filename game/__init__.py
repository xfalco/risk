...
class Game:
    ...
    def play_turn_start(self):
        ...
        menu = {"name": "Set territories", "options": {}}
        for continent in CONTINENTS:
            ...
            def click_country(country, continent=continent):
                ...
                menu["options"][continent.name()]["name"] = label_for_continent(
                    continent
                )
                menu["options"][continent.name()]["options"][country.name()]["name"] = (
                    label_for_country(country)
                )
                return True
            ...
            if neighbor_to_attack is None:
                ...
                def answer(outcome):
                    if outcome == WON_1_ARMY:
                        print(
                            "{} won {} with 1 army".format(
                                attacking_territory, neighbor_to_attack
                            )
                        )
                        player.gain_country(neighbor_to_attack, Strength.ONE_ARMY)
                    elif outcome == WON_2_ARMIES:
                        print(
                            "{} won {} with 2+ armies".format(
                                attacking_territory, neighbor_to_attack
                            )
                        )
